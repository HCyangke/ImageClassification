...
def get_class_labels():
    labels_dic={}
    index=0
    with open('/data/users/yangke/workplace/3D-ResNets-PyTorch-master/datasets/jester-v1-labels.csv','r') as myFile:
...
def get_video_names_and_annotations(subset):
    ...
    annotations = []
    n_frames = []
    file_path="/data/users/yangke/workplace/3D-ResNets-PyTorch-master/datasets/v1-{}.csv".format(subset)
    with open(file_path,'r') as myFile:
        lines=csv.reader(myFile)
        for line in lines:
            video_names.append(line[0])
...
def make_dataset(root_path, annotation_path, subset, n_samples_for_each_video,
    ...
    for i in range(len(video_names)):
        ...
        video_path = os.path.join('/data/users/yangke/workplace/flow_computation/sss/data/jester/20bn-dataset/20bn-jester-v1', 
                                video_names[i])
