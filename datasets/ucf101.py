...
def get_class_labels():
    labels_dic={}
    index=0
    with open('/nfs/project/UCF101/classInd.txt','r') as myFile:
        #lines=csv.reader(myFile)
        for line in myFile:
            line=line.strip().split(' ')[1]
            labels_dic[line]=index
            index+=1
    print(labels_dic)
    return labels_dic
...
def get_video_names_and_annotations(subset):
    ...
    annotations = []
    n_frames = []
    file_path="/nfs/project/UCF101/{}list01.csv".format(subset)
    with open(file_path,'r') as myFile:
        lines=csv.reader(myFile)
        for line in lines:
            #if line[1]!='BasketballDunk':
...
def make_dataset(root_path, annotation_path, subset, n_samples_for_each_video,
    ...
    for i in range(len(video_names)):
        ...
        video_path = os.path.join('/nfs/project/UCF101/UCF-101/{}/{}'.format(annotations[i],video_names[i]))
        #print(video_path)       
