...
if __name__ == '__main__':
    ...
    with open('data/list.csv') as f:
        for line in f:
            data = line.strip().split(',')
            images.extend([data])
    print('totle data: ',len(images))
    for image in images:
        img = Image.open(image[2]).convert("RGB")
        ...
        if label2index[ind.item()] != image[0]:
            # flag = ''
            print(label2index[ind.item()], image[2])
        # else:
            # flag = 'error'
